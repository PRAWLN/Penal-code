...
import { Search, Car, User, AlertTriangle, Sparkles, Siren, Gauge, RefreshCcw, Crosshair, Users, Briefcase, Anchor, Ticket, ShieldAlert, Trash2 } from 'lucide-react';

interface ScenarioSelectorProps {
  scenarioState: ScenarioState;
  onUpdate: (newState: Partial<ScenarioState>) => void;
  onAiAnalyze: (narrative: string) => Promise<void>;
  isAnalyzing: boolean;
}
...
export const ScenarioSelector: React.FC<ScenarioSelectorProps> = ({ 
  ...
  const [activeTab, setActiveTab] = useState<'quick' | 'narrative'>('quick');
  ...
  const toggleIncident = (id: string) => {
    ...
    if (current.includes(id)) {
      ...
      // Logic: If unchecking "Traffic Stop", also remove all sub-traffic violations
      if (id === 'traffic_stop') {
        const trafficIds = TRAFFIC_VIOLATIONS.map(t => t.id);
        nextIncidents = nextIncidents.filter(i => !trafficIds.includes(i));
        
        // Reset sub-state for traffic
        update.driverSpeed = '';
        update.trafficVehicleDestroyed = null;
      }

      // Logic: If unchecking Speeding, reset speed input
      if (id === 'traffic_speeding') {
        update.driverSpeed = '';
      }

      // Logic: If unchecking Stolen Vehicle (Traffic), reset destroyed status
      if (id === 'traffic_joyriding') {
        update.trafficVehicleDestroyed = null;
      }

      // Logic: If unchecking Shots Fired, reset all shots fired logic
      if (id === 'shots_fired') {
      ...
      // Logic: If removing littering, reset
      if (id === 'littering') {
        update.litteringRepeated = false;
      }

      // Logic: If removing fishing/hunting
      ...
      // Logic: If removing alarms that use hostages, potentially reset hostage count if no other alarm needs it
      ...
      if (remainingAlarms.length === 0 && alarmIds.includes(id)) {
          ...
          update.robberyStolenGoods = true; // Reset
      }

      // Logic: Reset warehouse robbery flag
      if (id === 'warehouse_robbery') {
          update.warehouseStolenGoods = true; // Reset to default
      }
      
      // Logic: Reset humane labs robbery flag
      if (id === 'humane_labs') {
          update.humaneLabsStolenGoods = true; // Reset to default
      }

      // GLOBAL RESET: If no incidents remain, reset shared sections (Suspect Behavior & Contraband)
  ...
  const handleFleeingChange = (val: FleeingType) => {
    ...
    // Logic: If changing away from Vehicle fleeing, reset all vehicle specific data
      ...
      <div className="flex bg-slate-800 p-1 rounded-lg border border-slate-700">
        <button
          ...
          className={`flex-1 py-2 text-sm font-medium rounded-md transition-all ${
            activeTab === 'quick' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'
          }`}
        >
          Quick Select
        </button>
        <button
          ...
          className={`flex-1 py-2 text-sm font-medium rounded-md transition-all flex items-center justify-center gap-2 ${
            activeTab === 'narrative' ? 'bg-purple-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'
          }`}
        >
          <Sparkles size={16} /> AI Narrative
        </button>
      </div>
        ...
        {activeTab === 'quick' ? (
          <>
            ...
            {/* --- INCIDENT SPECIFIC DETAILS --- */}
             {/* [Code for Warehouse, Humane Labs, Littering, Fishing, Hostage sections omitted for brevity as they are unchanged] */}
             
             {/* Conditional Warehouse Robbery Section */}
             ...
             {/* Conditional Humane Labs Robbery Section */}
             {scenarioState.incidentType.includes('humane_labs') && (
               <div className="mt-4 p-4 bg-slate-800/50 rounded-lg border border-slate-700 animate-in fade-in slide-in-from-top-2">
                  ...
                  <div className="flex flex-col gap-3">
                    <label className="text-xs text-slate-400 font-bold uppercase tracking-wider">Was suspect found with stolen goods from humane labs?</label>
                    <div className="flex gap-2">
                       <button
                  ...
                  <div className="flex flex-col gap-3">
                     <div className="bg-slate-900 p-3 rounded border border-slate-700">
                        ...
                        <p className="text-[10px] text-slate-500 mt-2 italic">
                           5th+ violation upgrades the charge to a Misdemeanor.
                        </p>
                     </div>
                  </div>
               </div>
             ...
             {/* Conditional Fishing / Hunting Section */}
                     ...
                     <div className="bg-slate-900 p-3 rounded border border-slate-700">
                        <label className="flex items-center cursor-pointer mb-2">
                           <input 
                             type="checkbox" 
                             className="mr-2"
                             checked={scenarioState.fishingViolation}
                             onChange={(e) => onUpdate({ fishingViolation: e.target.checked })}
                           />
                           ...
                           <div className="pl-6 space-y-3 animate-in fade-in slide-in-from-top-1">
                               ...
                               {!scenarioState.fishingHasLicense && (
                                   <label className="flex items-center justify-between text-xs text-slate-400">
                                      <span>Offense #</span>
                                      <select 
                                        className="bg-slate-800 border border-slate-600 rounded px-1 py-0.5 text-white"
                                        value={scenarioState.fishingOffenseNumber}
                                        onChange={(e) => onUpdate({ fishingOffenseNumber: parseInt(e.target.value) as 1|2|3 })}
                                      >
                               ...
                               <label className="flex items-center cursor-pointer">
                                  <input 
                                    type="checkbox" 
                                    className="mr-2"
                                    checked={scenarioState.fishingContainerViolation}
                                    onChange={(e) => onUpdate({ fishingContainerViolation: e.target.checked })}
                                  />
                     ...
                     <div className="bg-slate-900 p-3 rounded border border-slate-700">
                        <label className="flex items-center cursor-pointer mb-2">
                           <input 
                             type="checkbox" 
                             className="mr-2"
                             checked={scenarioState.huntingViolation}
                             onChange={(e) => onUpdate({ huntingViolation: e.target.checked })}
                           />
                           ...
                           <div className="pl-6 space-y-3 animate-in fade-in slide-in-from-top-1">
                               ...
                               <label className="flex items-center justify-between text-xs text-slate-400">
                                  ...
                                  <div className="flex bg-slate-800 rounded border border-slate-600">
                                     ...
                                     <button onClick={() => onUpdate({ huntingProtectedSpecies: false })} className={`px-2 py-0.5 ${!scenarioState.huntingProtectedSpecies ? 'bg-slate-600 text-white' : 'text-slate-500'}`}>No</button>
                                  </div>
                               </label>

                               {/* Specifics if in zone */}
                               ...
                               <div>
                                   ...
                                   <input 
                                     ...
                                     className="w-full bg-slate-800 border border-slate-600 rounded px-2 py-1 text-sm text-white focus:border-green-500 focus:outline-none"
                                     placeholder="Amount"
                                   />
                               </div>
             ...
             {showHostageSection && (
               <div className="mt-4 p-4 bg-slate-800/50 rounded-lg border border-slate-700 animate-in fade-in slide-in-from-top-2">
                  ...
                  {/* Stolen Goods Question */}
                  <div className="flex flex-col gap-3 mb-4 border-b border-slate-700 pb-4">
                    <label className="text-xs text-slate-400 font-bold uppercase tracking-wider">Was suspect found with stolen goods?</label>
                    <div className="flex gap-2">
                       <button
                         onClick={() => onUpdate({ robberyStolenGoods: true })}
                         className={`flex-1 py-2 text-xs font-bold rounded transition-colors ${scenarioState.robberyStolenGoods ? 'bg-red-600 text-white' : 'bg-slate-900 text-slate-400 border border-slate-700'}`}
                       >Yes (Principal)</button>
                       <button
                         onClick={() => onUpdate({ robberyStolenGoods: false })}
                         className={`flex-1 py-2 text-xs font-bold rounded transition-colors ${!scenarioState.robberyStolenGoods ? 'bg-slate-600 text-white' : 'bg-slate-900 text-slate-400 border border-slate-700'}`}
                       >No (Accessory)</button>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-3 mb-4 border-b border-slate-700 pb-4">
                    ...
                    <div className="flex gap-2">
                       <button
                         onClick={() => onUpdate({ robberyInjury: false })}
                         className={`flex-1 py-2 text-xs font-bold rounded transition-colors ${!scenarioState.robberyInjury ? 'bg-slate-600 text-white' : 'bg-slate-900 text-slate-400 border border-slate-700'}`}
                       >No</button>
                       <button
                         onClick={() => onUpdate({ robberyInjury: true })}
                         className={`flex-1 py-2 text-xs font-bold rounded transition-colors ${scenarioState.robberyInjury ? 'bg-red-600 text-white' : 'bg-slate-900 text-slate-400 border border-slate-700'}`}
                       >Yes</button>
                    </div>
                    {scenarioState.robberyInjury && (
                        <p className="text-[10px] text-red-400 italic">Upgrades to Aggravated Robbery.</p>
                    )}
                  </div>
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex gap-2">
                       <button
                         onClick={() => onUpdate({ hasHostages: false, hostageCount: '' })}
                         className={`flex-1 py-2 text-xs font-bold rounded transition-colors ${!scenarioState.hasHostages ? 'bg-slate-600 text-white' : 'bg-slate-900 text-slate-400 border border-slate-700'}`}
                       >No</button>
                       <button
                         onClick={() => onUpdate({ hasHostages: true, hostageCount: (typeof scenarioState.hostageCount === 'number' && scenarioState.hostageCount > 0) ? scenarioState.hostageCount : 1 })}
                         className={`flex-1 py-2 text-xs font-bold rounded transition-colors ${scenarioState.hasHostages ? 'bg-orange-600 text-white' : 'bg-slate-900 text-slate-400 border border-slate-700'}`}
                       >Yes</button>
                    </div>
                    ...
                    {scenarioState.hasHostages && (
                      <div className="animate-in fade-in slide-in-from-top-1">
                        ...
                        <input 
                          ...
                          className="w-full bg-slate-900 border border-slate-600 rounded px-2 py-1.5 text-sm text-white focus:border-orange-500 focus:outline-none"
                          placeholder="1"
                        />
                      </div>
                    )}
             ...
             {/* Conditional Traffic Violation Section */}
                      ...
                      <React.Fragment key={violation.id}>
                        <label 
                          className={`
                            ...
                            ${scenarioState.incidentType.includes(violation.id) ? 'bg-slate-700/50 text-white' : 'text-slate-400 hover:text-slate-300'}
                          `}
                        >
                           <input
                              type="checkbox"
                              className="hidden"
                              checked={scenarioState.incidentType.includes(violation.id)}
                              onChange={() => toggleIncident(violation.id)}
                            />
                          ...
                          <div className="ml-8 mb-2 p-3 bg-slate-900 rounded border border-slate-700 flex flex-col gap-3 animate-in fade-in slide-in-from-top-1">
                            <div className="flex justify-between items-center">
                               ...
                               <div className="flex bg-slate-800 rounded p-0.5 border border-slate-700">
                                 <button 
                                   onClick={() => onUpdate({ speedLimit: 75 })}
                                   className={`px-3 py-1 text-xs font-bold rounded transition-colors ${scenarioState.speedLimit === 75 ? 'bg-amber-600 text-white' : 'text-slate-400 hover:text-slate-200'}`}
                                 >75</button>
                                 <button 
                                   onClick={() => onUpdate({ speedLimit: 90 })}
                                   className={`px-3 py-1 text-xs font-bold rounded transition-colors ${scenarioState.speedLimit === 90 ? 'bg-amber-600 text-white' : 'text-slate-400 hover:text-slate-200'}`}
                                 >90</button>
                               </div>
                            </div>
                            <div className="flex justify-between items-center gap-2">
                               ...
                               <div className="relative w-24">
                                  <input 
                                    ...
                                    className="w-full bg-slate-800 border border-slate-600 rounded px-2 py-1 text-right text-sm font-mono text-white focus:outline-none focus:border-amber-500"
                                    placeholder="0"
                                  />
                            ...
                            {speedDiff > 0 && (
                               <div className="flex items-center gap-2 text-xs font-medium text-amber-400 bg-amber-900/20 p-2 rounded justify-center">
                                 <Gauge size={14} />
                                 {speedDiff} MPH over limit
                               </div>
                            )}
                          </div>
                        )}

                        {/* Stolen Vehicle (Joyriding) Sub-questions */}
                        {violation.id === 'traffic_joyriding' && scenarioState.incidentType.includes('traffic_joyriding') && scenarioState.suspectDriver !== 'mixed' && (
                          <div className="ml-8 mb-2 p-3 bg-slate-900 rounded border border-slate-700 animate-in fade-in slide-in-from-top-1">
                              ...
                              <div className="flex gap-2">
                                <button
                                  onClick={() => onUpdate({ trafficVehicleDestroyed: false })}
                                  className={`flex-1 py-1.5 text-xs font-bold rounded transition-colors ${scenarioState.trafficVehicleDestroyed === false ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'}`}
                                >No (Joyriding)</button>
                                <button
                                  onClick={() => onUpdate({ trafficVehicleDestroyed: true })}
                                  className={`flex-1 py-1.5 text-xs font-bold rounded transition-colors ${scenarioState.trafficVehicleDestroyed === true ? 'bg-red-600 text-white' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'}`}
                                >Yes (GTA)</button>
                              </div>
                          </div>
                        )}
             ...
             {/* Conditional Shots Fired Section */}
             {scenarioState.incidentType.includes('shots_fired') && (
               <div className="mt-4 p-4 bg-slate-800/50 rounded-lg border border-slate-700 animate-in fade-in slide-in-from-top-2">
                  ...
                  <div className="mb-4">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                       {[
                         ...
                         <button
                           key={opt.id}
                           onClick={() => onUpdate({ shotsFiredVictim: opt.id as any })}
                           className={`
                             py-2 px-3 text-xs font-semibold rounded border transition-all
                             ${scenarioState.shotsFiredVictim === opt.id 
                               ? 'bg-red-900/50 border-red-500 text-white' 
                               : 'bg-slate-900 border-slate-700 text-slate-400 hover:border-slate-600'}
                             ${opt.id === 'govt' ? 'col-span-2' : ''}
                           `}
                         >
                           {opt.label}
                         </button>
                    ...
                    <div className="mb-4 p-3 bg-slate-900 rounded border border-slate-700 animate-in fade-in slide-in-from-top-1 space-y-3">
                       ...
                       <div>
                          ...
                          <input 
                            ...
                            className="w-full bg-slate-800 border border-slate-600 rounded px-2 py-1 text-sm text-white focus:border-red-500 focus:outline-none"
                            placeholder="1"
                          />
                       </div>

                       {/* Govt Specific Question */}
                       {scenarioState.shotsFiredVictim === 'govt' && (
                         <div>
                            ...
                            <div className="flex gap-2">
                               <button
                                 onClick={() => onUpdate({ shotsFiredGovtActive: false })}
                                 className={`flex-1 py-1.5 text-xs font-bold rounded transition-colors ${!scenarioState.shotsFiredGovtActive ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-400'}`}
                               >No (Assault Govt)</button>
                               <button
                                 onClick={() => onUpdate({ shotsFiredGovtActive: true })}
                                 className={`flex-1 py-1.5 text-xs font-bold rounded transition-colors ${scenarioState.shotsFiredGovtActive ? 'bg-red-600 text-white' : 'bg-slate-800 text-slate-400'}`}
                               >Yes (Agg Assault)</button>
                            </div>
                         </div>
                       )}
                    </div>
                  ...
                  <div className="pt-2 border-t border-slate-700">
                     ...
                     <div className="flex gap-2">
                        <button
                           onClick={() => onUpdate({ shotsFiredRole: 'principal' })}
                           className={`flex-1 py-1.5 text-xs font-bold rounded transition-colors ${scenarioState.shotsFiredRole === 'principal' ? 'bg-slate-600 text-white' : 'bg-slate-800 text-slate-400'}`}
                        >Shooter (Principal)</button>
                        <button
                           onClick={() => onUpdate({ shotsFiredRole: 'accessory' })}
                           className={`flex-1 py-1.5 text-xs font-bold rounded transition-colors ${scenarioState.shotsFiredRole === 'accessory' ? 'bg-slate-600 text-white' : 'bg-slate-800 text-slate-400'}`}
                        >Accessory</button>
                     </div>
                  </div>
               </div>
            ...
            {/* --- END INCIDENT SPECIFIC DETAILS --- */}
            
            
            {/* --- CONDITIONAL SECTIONS (Visible only if incidents selected) --- */}
            {hasActiveIncidents && (
              <>
                <div className="h-px bg-slate-700 my-4" />

                {/* Section 2: Evasion (Suspect Behavior) */}
                    ...
                    <div className="mt-4 p-4 bg-slate-800/50 rounded-lg border border-slate-700 animate-in fade-in slide-in-from-top-2">
                      ...
                      <div className="grid grid-cols-1 gap-4">
                        <div>
                          ...
                          <div className="flex gap-2">
                             <button
                               onClick={() => onUpdate({ suspectDriver: 'no' })}
                               className={`
                                 flex-1 py-1.5 px-2 rounded border text-xs font-semibold uppercase tracking-wide transition-colors
                                 ${scenarioState.suspectDriver === 'no'
                             ...
                             <button
                               onClick={() => onUpdate({ suspectDriver: 'yes' })}
                               className={`
                                 flex-1 py-1.5 px-2 rounded border text-xs font-semibold uppercase tracking-wide transition-colors
                                 ${scenarioState.suspectDriver === 'yes'
                                   ? 'bg-red-900/40 border-red-500 text-red-200'
                                   : 'bg-slate-900 border-slate-700 text-slate-500 hover:text-slate-300'}
                               `}
                             >
                             ...
                             <button
                               onClick={() => onUpdate({ suspectDriver: 'mixed' })}
                               className={`
                                 flex-1 py-1.5 px-2 rounded border text-xs font-semibold uppercase tracking-wide transition-colors
                        ...
                        {scenarioState.suspectDriver === 'mixed' && (
                          <div className="mt-2 bg-slate-900 rounded border border-slate-700 p-3 animate-in fade-in slide-in-from-top-1">
                                ...
                                <div>
                                   ...
                                   <div className="flex items-center gap-1">
                                     ...
                                     <input 
                                      ...
                                      value={scenarioState.vehicleTheftPrincipalRecovered}
                                      onChange={(e) => onUpdate({ vehicleTheftPrincipalRecovered: e.target.value === '' ? '' : parseInt(e.target.value) })}
                                      className="w-full bg-slate-800 border border-slate-600 rounded px-1.5 py-0.5 text-xs text-white focus:border-blue-500 focus:outline-none"
                                     />
                                   </div>
                                </div>
                                <div>
                                   ...
                                   <div className="flex items-center gap-1">
                                     ...
                                     <input 
                                      type="number" min="0" placeholder="0"
                                      value={scenarioState.vehicleTheftPrincipalDestroyed}
                                      onChange={(e) => onUpdate({ vehicleTheftPrincipalDestroyed: e.target.value === '' ? '' : parseInt(e.target.value) })}
                                      className="w-full bg-slate-800 border border-slate-600 rounded px-1.5 py-0.5 text-xs text-white focus:border-red-500 focus:outline-none"
                                     />
                                   </div>
                                </div>
                              </div>
                              ...
                              <div className="space-y-2">
                                ...
                                <div>
                                   ...
                                   <div className="flex items-center gap-1">
                                     ...
                                     <input 
                                      type="number" min="0" placeholder="0"
                                      value={scenarioState.vehicleTheftAccessoryRecovered}
                                      onChange={(e) => onUpdate({ vehicleTheftAccessoryRecovered: e.target.value === '' ? '' : parseInt(e.target.value) })}
                                      className="w-full bg-slate-800 border border-slate-600 rounded px-1.5 py-0.5 text-xs text-white focus:border-blue-500 focus:outline-none"
                                     />
                                   </div>
                                </div>
                                <div>
                                   ...
                                   <div className="flex items-center gap-1">
                                     ...
                                     <input 
                                      type="number" min="0" placeholder="0"
                                      value={scenarioState.vehicleTheftAccessoryDestroyed}
                                      onChange={(e) => onUpdate({ vehicleTheftAccessoryDestroyed: e.target.value === '' ? '' : parseInt(e.target.value) })}
                                      className="w-full bg-slate-800 border border-slate-600 rounded px-1.5 py-0.5 text-xs text-white focus:border-red-500 focus:outline-none"
                                     />
                                   </div>
                                </div>
                            ...
                            <p className="text-[9px] text-slate-500 mt-2 italic text-center">Enter total counts for ALL involved vehicles.</p>
                          </div>
                        )}

                        {/* Standard Driver Logic (Yes/Mixed) - Reckless Damage */}
                        {(scenarioState.suspectDriver === 'yes' || scenarioState.suspectDriver === 'mixed') && (
                            <div className="mt-2 p-3 bg-slate-900 rounded border border-slate-700 animate-in fade-in slide-in-from-top-1">
                                ...
                                <div className="flex gap-2">
                                  <button
                                    onClick={() => onUpdate({ recklessEvasionDamage: false })}
                                    className={`flex-1 py-1.5 text-xs font-bold rounded transition-colors ${!scenarioState.recklessEvasionDamage ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'}`}
                                  >No</button>
                                  <button
                                    onClick={() => onUpdate({ recklessEvasionDamage: true })}
                                    className={`flex-1 py-1.5 text-xs font-bold rounded transition-colors ${scenarioState.recklessEvasionDamage ? 'bg-red-600 text-white' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'}`}
                                  >Yes (Reckless)</button>
                                </div>
                                {scenarioState.recklessEvasionDamage && <p className="text-[10px] text-red-400 mt-1 italic">Upgrades Evading to Reckless Evading</p>}
                            </div>
                        )}

                        {/* Standard Vehicle Swaps Logic (Hidden if Mixed) */}
                        {scenarioState.suspectDriver !== 'mixed' && (
                            <div className="pt-2 border-t border-slate-700">
                               <span className="text-sm text-slate-300 block mb-2">Was any vehicles during this pursuit stolen and documented?</span>
                               <div className="flex gap-2 mb-3">
                                 <button
                                   onClick={() => handleVehicleSwapsChange(false)}
                                   className={`flex-1 py-1.5 text-xs font-bold rounded transition-colors ${!scenarioState.vehicleSwaps ? 'bg-slate-600 text-white' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'}`}
                                 >No</button>
                                 <button
                                   onClick={() => handleVehicleSwapsChange(true)}
                                   className={`flex-1 py-1.5 text-xs font-bold rounded transition-colors ${scenarioState.vehicleSwaps ? 'bg-red-600 text-white' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'}`}
                                 >Yes</button>
                               </div>
                                        ...
                                        <div>
                                           ...
                                           <div className="flex items-center gap-1">
                                             ...
                                             <input 
                                              ...
                                              min="0"
                                              placeholder="0"
                                              value={scenarioState.stolenRecovered}
                                              onChange={(e) => onUpdate({ stolenRecovered: e.target.value === '' ? '' : parseInt(e.target.value) })}
                                           ...
                                           <span className="text-[10px] text-slate-500">Counts as Joyriding</span>
                                        </div>
                                        <div>
                                           ...
                                           <div className="flex items-center gap-1">
                                             ...
                                             <input 
                                              ...
                                              min="0"
                                              placeholder="0"
                                              value={scenarioState.stolenDestroyed}
                                              onChange={(e) => onUpdate({ stolenDestroyed: e.target.value === '' ? '' : parseInt(e.target.value) })}
                                              className="w-full bg-slate-800 border border-slate-600 rounded px-2 py-1 text-xs text-white focus:border-red-500 focus:outline-none"
                                             />
                                           </div>
                                           <span className="text-[10px] text-slate-500">Counts as GTA</span>
                                        </div>
                                     </div>
                                     <p className="text-[10px] text-slate-400/70 mt-3 text-center italic border-t border-slate-800 pt-2">
                                       All vehicles MUST be stolen and documented in the report
                                     </p>
                                 </div>
                               )}
                            </div>
                ...
                {/* Section 2.5: Officer Safety & Resistance */}
                    ...
                    <div className="bg-slate-800/30 p-3 rounded border border-slate-700">
                        <div className="flex flex-col gap-3 mb-3">
                           ...
                           <div className="flex gap-2">
                              <button
                                ...
                                className={`flex-1 py-2 text-xs font-bold rounded transition-colors ${!scenarioState.officerAttack ? 'bg-slate-600 text-white' : 'bg-slate-900 text-slate-400 border border-slate-700'}`}
                              >No</button>
                              <button
                                onClick={() => onUpdate({ officerAttack: true })}
                                className={`flex-1 py-2 text-xs font-bold rounded transition-colors ${scenarioState.officerAttack ? 'bg-red-600 text-white' : 'bg-slate-900 text-slate-400 border border-slate-700'}`}
                              >Yes</button>
                           </div>
                        </div>
                        ...
                        {scenarioState.officerAttack && (
                           <div className="mt-4 pl-1 space-y-4 animate-in fade-in slide-in-from-top-1 border-t border-slate-700/50 pt-3">
                               ...
                               {/* Question 2: Witnessed/GSR - Only if contact was made */}
                               <div className="animate-in fade-in slide-in-from-top-1">
                                  ...
                                  <div className="flex gap-2">
                                     <button 
                                       ...
                                       className={`flex-1 py-1.5 text-xs font-bold rounded transition-colors ${scenarioState.officerAttackGSR ? 'bg-blue-600 text-white' : 'bg-slate-900 text-slate-400 border border-slate-700'}`}
                                     >Yes (Principal)</button>
                                     <button 
                                       onClick={() => onUpdate({ officerAttackGSR: false })} 
                                       className={`flex-1 py-1.5 text-xs font-bold rounded transition-colors ${!scenarioState.officerAttackGSR ? 'bg-slate-600 text-white' : 'bg-slate-900 text-slate-400 border border-slate-700'}`}
                                     >No (Accessory)</button>
                                  </div>
                               </div>
                               ...
                               <div className="grid grid-cols-1 gap-3 pt-2">
                                   <div>
                                       ...
                                       <div className="relative">
                                            <input 
                                              type="number"
                                              min="0"
                                              value={scenarioState.officerAttackCountWeapon}
                                              onChange={(e) => onUpdate({ officerAttackCountWeapon: e.target.value === '' ? '' : parseInt(e.target.value) })}
                                              className="w-full bg-slate-900 border border-slate-600 rounded px-2 py-1.5 text-sm text-white focus:border-red-500 focus:outline-none pl-8"
                                              placeholder="0"
                                            />
                                            <ShieldAlert size={14} className="absolute left-2 top-1/2 -translate-y-1/2 text-red-500" />
                                       </div>
                                       <p className="text-[10px] text-slate-500 mt-0.5">Aggravated Assault & Battery</p>
                                   </div>
                                   ...
                                   <div>
                                       ...
                                       <input 
                                         type="number"
                                         min="0"
                                         value={scenarioState.officerAttackCountNoWeapon}
                                         onChange={(e) => onUpdate({ officerAttackCountNoWeapon: e.target.value === '' ? '' : parseInt(e.target.value) })}
                                         className="w-full bg-slate-900 border border-slate-600 rounded px-2 py-1.5 text-sm text-white focus:border-orange-500 focus:outline-none"
                                         placeholder="0"
                                       />
                                       <p className="text-[10px] text-slate-500 mt-0.5">Assault & Battery</p>
                                   </div>
                                   ...
                                   <div>
                                       ...
                                       <input 
                                         type="number"
                                         min="0"
                                         value={scenarioState.officerAttackCountTargeted}
                                         onChange={(e) => onUpdate({ officerAttackCountTargeted: e.target.value === '' ? '' : parseInt(e.target.value) })}
                                         className="w-full bg-slate-900 border border-slate-600 rounded px-2 py-1.5 text-sm text-white focus:border-blue-500 focus:outline-none"
                                         placeholder="0"
                                       />
                                       <p className="text-[10px] text-slate-500 mt-0.5">Assault on a Govt Official</p>
                                   </div>
                               </div>
                           </div>
                        )}
                   ...
                   <div className="bg-slate-800/30 p-3 rounded border border-slate-700 flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                          ...
                          <input 
                            type="number" 
                            placeholder="0" 
                            className="w-16 bg-slate-900 border border-slate-600 rounded px-2 py-1 text-xs text-right"
          ...
          <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
             {/* [AI Narrative tab content unchanged] */}
            <div className="bg-purple-900/20 border border-purple-500/30 p-4 rounded-lg">
              <h3 className="text-purple-300 font-semibold mb-2 flex items-center gap-2">
                <Sparkles size={18} /> AI Investigator
              </h3>
              <p className="text-xs text-purple-200/70 mb-4">
                Paste the witness statement or describe the scenario below. The AI will analyze the narrative against the Penal Code to suggest charges.
              </p>
              
              <textarea
                value={scenarioState.customNarrative}
                onChange={(e) => onUpdate({ customNarrative: e.target.value })}
                placeholder="e.g., Suspect was seen breaking into the warehouse on Vespucci Blvd. When officers arrived, he fled in a black Gauntlet which was later confirmed stolen. He crashed into a civ vehicle during the chase."
                className="w-full h-48 bg-slate-900 border border-slate-700 rounded-md p-3 text-sm text-slate-200 placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-purple-500 resize-none"
              />
              
              <button
                disabled={isAnalyzing || !scenarioState.customNarrative.trim()}
                onClick={() => onAiAnalyze(scenarioState.customNarrative)}
                className={`
                  w-full mt-4 py-2 px-4 rounded-md font-semibold text-white transition-all
                  ${isAnalyzing || !scenarioState.customNarrative.trim()
                    ? 'bg-slate-700 text-slate-400 cursor-not-allowed'
                    : 'bg-purple-600 hover:bg-purple-500 shadow-lg shadow-purple-900/20'}
                `}
              >
                {isAnalyzing ? 'Analyzing Narrative...' : 'Analyze & Suggest Charges'}
              </button>
            </div>
          </div>
        )}
